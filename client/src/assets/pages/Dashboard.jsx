...
import { dummyResumeData } from '../assets.js';
...
const Dashboard = () => {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-8">
      {/* Welcome Text */}
      <p className="medium text-2xl font-bold bg-gradient-to-r from-slate-600 to-slate-700 bg-clip-text text-transparent sm:hidden">
        Welcome, John Doe
      </p>
      ...
      <div className="flex gap-4 mt-4">
        ...
        <button onClick={() => setShowCreateResume(true)}
          ...
          text-slate-600 border border-dashed border-slate-300 group 
      ...
      <div className="grid grid-cols-2 sm:flex flex-wrap gap-4">
        ...
        {showUploadResume && (
            <form onSubmit={uploadResume} onClick={()=>setShowUploadResume(false)}  className ='fixed inset-0 bg-black/70 backdrop-blur bg-opacity-50 z-10 flex items-center justify-center'>
                <div onClick={e => e.stopPropagation()} className='relative bg-slate-50 border shadow-md rounded-lg w-full max-w-sm p-6'>
                    <h2 text-xl font-bold mb-4>Upload Resume</h2>
                    <input onChange={(e) => setTitle(e.target.value)} value={title} type ="text" placeholder="Resume Title" className="w-full px-4 py-2 mb-4 focus:border-green-600 ring-green-600" required />
                    ...
                    <div>
                        <label htmlFor='resume-input' className="block text-sm text-slate-700">
                            ...
                            <div className='flex flex-col items-center justify-center gap-2 border group text-slate-400
                             border-slate-400 border-dashed rounded-md p-4 py-10 my-4 hover: border green-500 hover:text-green-700 cursor transition-colors'>
                                {resume ?(
